<template>
    <div class="quotation-card">
        ...
        <div class="quotation-header mb-3">
            <h5>Quotation Expedition | Detail</h5>
            <hr />
        </div>
        ...
        <div class="quotation-table mt-3">
            <table class="table table-bordered table-responsive">
                ...
                <tbody>
                    <tr v-for="(item, index) in dataApproval" :key="index">
                        ...
                        <td>
                            ...
                            <span
                                :class="{
                                    'badge bg-danger': item.Flag === 'R',
                                    'badge bg-success': item.Flag === 'A',
        ...
        async fetchDetailData(NoPenawaran) {
            try {
                const response = await axios.post(
                    `${SRV_URL}approval_expedition/get_approval_detail`,
                    {
                ...
                if (response.data.data.status === 200) {
                    this.detailData = response.data.data.data; // Store API response in detailData
                } else {
                    console.error(
