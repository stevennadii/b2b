<template>
    <div class="quotation-card">
        ...
        <div class="quotation-header mb-3">
            <h5>Approval Raw Material | Compare</h5>
            <hr />
        </div>
            ...
            <div class="filter-items d-flex align-items-center">
                <div class="filter-item ml-2">
                    ...
                    <select id="name" v-model="filters.name" class="px-3">
                        ...
                        <option
                            v-for="data in dataFilterBarang"
                            :key="data.Nama"
                            :value="data.Nama"
                        >
                            {{ data.Nama }}
        ...
        <!-- Table Section (remains same as before) -->
        ...
        <!-- Buttons Section (remains same as before) -->
        <div class="row">
            <div class="col-md-6">
...
export default {
    ...
    data() {
        return {
            ...
            selectedActions: {}, // Menyimpan hasil filter
        };
    },
    mounted() {
        this.getTodayDate();
    ...
    methods: {
        async save() {
            ...
            formData.append("noApproval", this.noApproval);

            // Kirim formData ke API menggunakan axios
            try {
                this.isLoading = true;
                const response = await axios.post(
                    `${SRV_URL}approval_rawmaterial/simpan_approval_compare`,
                    formData,
                    {
                        headers: {
        ...
        getTodayDate() {
            ...
            this.tanggal = new Intl.DateTimeFormat("en-GB", options).format(
                date
            );
        },
        applyFilter() {
            // Filter data hanya jika nama barang diisi
            let filtered = this.filters.name
