<template>
    <div class="quotation-card">
        ...
        <div class="quotation-header mb-3">
            <h5>Approval Raw Material | Detail</h5>
            <hr />
        </div>
...
export default {
    ...
    methods: {
        async fetchDetailData(NoPenawaran) {
            // const nomorPenawaran = NoPenawaran.replace("/", "@");
            // console.log(nomorPenawaran);
            try {
                const response = await axios.post(
                    `${SRV_URL}approval_rawmaterial/get_approval_detail_user`,
                    {
